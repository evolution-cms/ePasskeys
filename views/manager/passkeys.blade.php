...
<div class="epasskeys {{ $themeClass ?? '' }}" data-epasskeys-theme="{{ $themeMode ?? 'auto' }}">
    <div class="mx-auto flex max-w-5xl flex-col gap-6 px-4 py-6 sm:px-6">
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                <p class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400 darkness:text-slate-500">Security</p>
                <h1 class="mt-1 text-2xl font-semibold text-slate-900 darkness:text-slate-100">Passkeys</h1>
                <p class="mt-2 max-w-xl text-sm text-slate-500 darkness:text-slate-400">
                    Use passkeys to sign in without a password. Manage devices and revoke access anytime.
                </p>
            </div>
        ...
        <div class="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
            <section class="rounded-2xl border border-slate-200/70 bg-white/80 p-6 shadow-sm backdrop-blur darkness:border-slate-700/60 darkness:bg-slate-900/60">
                <div class="flex items-start justify-between gap-3">
                    <div>
                        <h3 class="text-lg font-semibold text-slate-900 darkness:text-slate-100">Create passkey</h3>
                ...
                <form id="epasskeys-register-form" method="POST" action="{{ $registerUrl }}" class="mt-5 space-y-4">
                    {!! csrf_field() !!}
                    <div>
                        <label for="epasskeys-name" class="text-xs font-semibold uppercase tracking-wide text-slate-500 darkness:text-slate-400">Device name</label>
                        <input
                            type="text"
                            id="epasskeys-name"
                            name="name"
                    ...
                    <div class="flex flex-wrap items-center justify-between gap-3">
                        <p class="text-xs text-slate-500 darkness:text-slate-400">
                            You will be prompted to confirm the passkey on this device.
                        </p>
            ...
            <section class="rounded-2xl border border-slate-200/70 bg-white/80 p-6 shadow-sm backdrop-blur darkness:border-slate-700/60 darkness:bg-slate-900/60">
                <div class="flex items-center justify-between gap-3">
                    <h3 class="text-lg font-semibold text-slate-900 darkness:text-slate-100">Existing passkeys</h3>
                    <span class="text-xs font-medium text-slate-400 darkness:text-slate-500">{{ $passkeys->count() }} active</span>
                </div>
                ...
                @if ($passkeys->isEmpty())
                    <div class="mt-4 rounded-xl border border-dashed border-slate-200/80 bg-slate-50/70 p-4 text-sm text-slate-500 darkness:border-slate-700/70 darkness:bg-slate-900/40 darkness:text-slate-400">
                        No passkeys yet. Create your first one to enable passwordless login.
                    </div>
                @else
                                ...
                                @foreach ($passkeys as $passkey)
                                    <tr class="text-slate-700 darkness:text-slate-200">
                                        ...
                                        <td class="py-3 pr-4 text-sm text-slate-500 darkness:text-slate-400">
                                            {{ $passkey->last_used_at ? $passkey->last_used_at->diffForHumans() : 'Not used yet' }}
                                        </td>
                                        <td class="py-3 pr-4 text-sm text-slate-500 darkness:text-slate-400">
